...
<html>
	...
	<body class="renew-book">
		...
		<div class="form">
			<h2>Renew Book</h2>
			<form method="post" action="Includes/RenewBook.inc.php">
				<p id="text">
					<?php
						if(isset($_GET['text'])==False)
						{
							header("Location: RenewBook.php?text=");
							exit();
						}
						$text = $_GET['text'];
						echo("$text");
			    ...
			    <label><b>New Return Date</b></label> 
			    <input type="date" name="new" required>

				<button type="renew" name="submit" value="Submit">Renew</button>
			</form>
		</div>
	</body>
</html>